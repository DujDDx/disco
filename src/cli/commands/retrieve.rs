...
pub fn handle_retrieve_with_ctx(ctx: &AppContext, keyword: String) -> Result<()> {
    ...
    if files.is_empty() {
        println!();
        print_warning("No files found matching the keyword.");
        return Ok(());
    }

    println!();
    print_header("Found Files:");
    println!();
    ...
    // Ask which files to retrieve
    print!("Enter file numbers to retrieve (e.g., 1,3,5) or 'all': ");
    io::stdout().flush().ok();
    ...
    for &idx in &indices {
        ...
        // Get disk info
        let disk = match disk_repo.get_disk_by_id(&entry.disk_id) {
            Ok(d) => d,
            Err(e) => {
                println!("  {} Error: {}", entry.file_name.red(), e);
                failed_count += 1;
                continue;
            }
        };
        ...
        let mount = match current_mount {
            Some(m) => m,
            None => {
                println!("  {} Disk '{}' not mounted, skipping", entry.file_name.red(), disk.name);
                failed_count += 1;
                continue;
            }
        };
        ...
        println!("  Copying {}...", entry.file_name.cyan());

        // Copy with progress
        match copy_file_with_progress(&source_path, &dest_path) {
            Ok(size) => {
                println!("    ✓ Saved to {}", dest_path.display().to_string().green());
                success_count += 1;
                total_size += size;
            }
            Err(e) => {
